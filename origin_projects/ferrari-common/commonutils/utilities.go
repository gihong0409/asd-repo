...
func JsonToHader(jsonOrg []byte, headerObj interface{}) {
	jsonData := make(map[string]interface{})
...
func JsonToBody(jsonOrg []byte, bodyObj interface{}) {
	...
	if err := json.Unmarshal(bodyBytes, &bodyObj); err != nil {
		fmt.Print("ERROR line utilites266", err, strJson)
	}
}
...
//RequestPassResByte request uri to tls 12 return byte arrays
func RequestPassResByte(uri string, data []byte) []byte {
	tr := &http.Transport{
		TLSClientConfig: &tls.Config{
			MinVersion:         tls.VersionTLS12,
			InsecureSkipVerify: true,
	...
	nano := "000"
	nanoInt := t.Nanosecond()

	if nanoInt < 100000 {
		nano = fmt.Sprintf("%06d\n", nanoInt)
	} else {
		nano = strconv.Itoa(nanoInt)
	}
	nano = nano[len(nano)-6 : len(nano)-3]
	id := now + nano
	tKey := now + nano + "ANTISCAM"
	key := strings.ToUpper(NewMD5([]byte(tKey)).String())
	req.SetBasicAuth(id, key)

	req.Header["x-ext-sessionId"] = []string{GUIDGen(16)}
...
func GetClientIPByHeaders(r *http.Request) string {
...
// UnmarshalCp949XML xml unmarshal cp949 / euc-kr
...
func SendEarthNotify(cmdType int, batchType int, serviceName string, serverName string, platformName string, msg string, ConfigSet string) error {
	var notifyData []byte
	var notifyURL string

	if cmdType == notifyformats.SLACK_MSG_BYPASS {
		return errors.New("cmdType error") // 에러 던지기
	}
	...
	reqHeader := notifyformats.ReqHeader{CmdType: cmdType}
	reqBodyCommon := notifyformats.ReqBodySMCommon{ServiceName: serviceName, ServerName: serverName, Platform: platformName, Msg: msg}

	var reqBody interface{}
	if cmdType == notifyformats.DB_INSERT {
		reqBody = notifyformats.ReqBodyDBInsert{
			ReqBodySMCommon: reqBodyCommon,
			BatchType:       batchType,
		}
	} else {
		reqBody = notifyformats.ReqBodyWebhookNotification{ReqBodySMCommon: reqBodyCommon}
	}
	...
	if err != nil {
		return fmt.Errorf("error marshalling combined data: %v", err)
	}
...
func SendSlackMsgByPass(msg string, channelID string, ConfigSet string) error {
	var notifyData []byte
	var notifyURL string
	...
	reqHeader := notifyformats.ReqHeader{CmdType: notifyformats.SLACK_MSG_BYPASS}
